...
export function DocumentEditor({
  ...
  const handleSlashSelect = useCallback((item: SlashCommandItem, tiptapCommand: ((item: SlashCommandItem) => void) | null) => {
    ...
    if (item.blockType === 'divider') {
      ...
      onUpdateBlock(currentId, { block_type: item.blockType as BlockType });
    }
        ...
        return (
          <div
            ...
            className={[
              ...
              isDragOver && dragPos === 'top'    ? 'border-t-2 border-violet-500' : '',
              isDragOver && dragPos === 'bottom' ? 'border-b-2 border-violet-500' : '',
            ].join(' ')}
            onMouseMove={(e) => {
              if (isDraggingRef.current) return;
